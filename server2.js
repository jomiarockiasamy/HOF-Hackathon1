...
const dbPath = path.join(__dirname, 'requests.db'); // Make sure it's .db not .sql
...
app.get('/requests', (req, res) => {
    const query = 'SELECT * FROM HelpRequests'; // Make sure the table name matches
...
app.get('/requests/:id', (req, res) => {
    const query = 'SELECT * FROM HelpRequests WHERE requestId = ?';
...
app.put('/requests/:id', (req, res) => {
    ...
    const query = 'UPDATE HelpRequests SET title = ?, description = ? WHERE requestId = ?';
...
app.delete('/requests/:id', (req, res) => {
    const query = 'DELETE FROM HelpRequests WHERE requestId = ?';
    const params = [req.params.id];
    db.run(query, params, function (err) {
        if (err) {
            res.status(500).json({ error: err.message });
